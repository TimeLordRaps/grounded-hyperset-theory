...
class PermutationGroup:
    ...
    def orbits(self, nodes: Iterable[Node | Hashable] | None = None) -> list[set[Node]]:
        """Compute the orbits of the group action on the domain."""
        ...
        for n in sorted(target_nodes, key=lambda x: str(x.id)):
            if n not in visited:
                orb = {g.apply(n) for g in self._elements if g.apply(n) in target_nodes}
                visited.update(orb)
                result.append(orb)
...
def orbit(node: Node | Hashable, group: Iterable[Permutation]) -> set[Node]:
    """Return the orbit of node under the action of group."""
    n = node if isinstance(node, Node) else Node(node)
    return {g.apply(n) for g in group}


def all_orbits(apg: AccessiblePointedGraph, group: Iterable[Permutation]) -> list[set[Node]]:
    """Return all orbits partition of nodes in apg under group."""
    ...
    for n in sorted(target_nodes, key=lambda x: str(x.id)):
        if n not in visited:
            orb = {g.apply(n) for g in g_list if g.apply(n) in target_nodes}
            visited.update(orb)
            result.append(orb)
